...
$stmt = $con->prepare('SELECT mot_de_passe, email, nom, prenom FROM compte WHERE id = ?');
// In this case we can use the account ID to get the account info.
$stmt->bind_param('i', $_SESSION['id']);
$stmt->execute();
$stmt->bind_result($password, $email, $nom, $prenom);
$stmt->fetch();
$stmt->close();
?>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="inner-content">
                        ...
                        <span> veuillez souscrire a l'une des conditions pour etre membre<?=$password?></span>
                    </div>
                </div>
            </div>
        </div>
