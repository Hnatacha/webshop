...
//We need to check name invalid characters validation
if (preg_match('/^[a-zA-Z0-9]+$/', $_POST['nom']) == 0 || preg_match('/^[a-zA-Z0-9]+$/', $_POST['prenom']) == 0) {
    exit('last name or firstname is not valid!');
}
...
if ($stmt = $con->prepare('SELECT id, mot_de_passe FROM compte WHERE email = ?')) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO compte (nom, prenom, password, email) VALUES (?, ?, ?)')) {
            ...
            $activate_link = 'http://localhost.com/webshop/activate.php?email=' . $_POST['email'] . '&code=' . $uniqid;
