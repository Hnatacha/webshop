        ...
        <section class="section" id="products">
          <table class="table table-striped">
            ...
            <tbody>
              ...
              <tr>
                <th scope="row"><?=$k?></th>
                <td><img src="../<?=$produit['url']?>" alt="" width="60px"></td>
                <td><?=$produit['libelle']?></td>
                <td><?=$produit['prix']?></td> 
                <td><?=$produit['description']?></td>
                <td><?=$produit['rate']?></td>
                <td><?=$produit['nom_categorie']?></td> 
                <td><?=$produit['nom_saison']?></td>
                <td><?=$produit['quantite']?></td>
                <td>
                  <span>
                  <a href="edit.php?id=<?=$produit['id']?>" class="text-primary" title="Editer">
                        <i class="fa fa-edit" aria-hidden="true" data-id="<?=$produit['id']?>" ></i>
                      </a>
                  <a href="#" class="text-danger ml-2" title="Supprimer" data-id="<?=$produit['id']?>"  
                         data-libelle="<?=$produit['libelle']?>" onclick="onSupprimer(event)">
                        <i class="fa fa-trash" aria-hidden="true" data-id="<?=$produit['id']?>"  
                         data-libelle="<?=$produit['libelle']?>" ></i>
                      </a>
                  </span></td>
              </tr>
              <?php
    ...
    <script>
      $('#')
       onSupprimer = (event) => {
             ...
             let libelle = $(target).data("libelle");

             let mBody = document.getElementById('suppressionMBody');
             mBody.innerHTML = '';
             let body = libelle +'<br><br> Supprimer ?';
                 body += '<input type="text" hidden value="'+id+'" name="productID" id="productID">';
