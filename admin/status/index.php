    ...
    <div class="wrapper d-flex flex-column min-vh-100 bg-light">
      ...
      <div class="body flex-grow-1 px-3">
        <div class="d-flex flex-row-reverse">
          <a href="edit.php" class="btn btn-primary" title="Ajouter un status_commande">
        ...
        <section class="section" id="products">
          <table class="table table-striped">
            ...
            <tbody>
              <?php
              foreach($status_commandes as $k => $status_commande)
              {
              ?>
              <tr>
                <th scope="row"><?=$k?></th>
                <td><?=$status_commande['nom']?></td>
                <td>
                  <span>
                  <a href="edit.php?id=<?=$status_commande['id']?>" class="text-primary" title="Editer">
                        <i class="fa fa-edit" aria-hidden="true" data-id="<?=$status_commande['id']?>" ></i>
                      </a>
                  <a href="#" class="text-danger ml-2" title="Supprimer" data-id="<?=$status_commande['id']?>"  
                         data-nom="<?=$status_commande['nom']?>" onclick="onSupprimer(event)">
                        <i class="fa fa-trash" aria-hidden="true" data-id="<?=$status_commande['id']?>"  
                         data-nom="<?=$status_commande['nom']?>" ></i>
                      </a>
                  </span></td>
              </tr>
              <?php
      ...
      $('#')
       onSupprimer = (event) => {
             ...
             let body = nom +'<br><br> Supprimer ?';
                 body += '<input type="text" hidden value="'+id+'" name="status_commandeID" id="status_commandeID">';
        ...
        supprimer =  (event) => {
          let id = $('#status_commandeID').val();
          var formData = new FormData();
          formData.append('_method', 'DELETE');
          formData.append('id', id);

          request = $.ajax({
          url: "status_commandeController.php",
          type:  'POST',
          data: formData,
          cache: false,
          contentType: false,
        ...
        request.done(function(response, textStatus, jqXHR) {
          ...
          if(response.success){
            ...
            toastr.success('Status_commande supprimé');
            setTimeout(() => {
                window.location.href = window.location.origin+'/webshop/admin/status/index.php';
              }, 1000);
