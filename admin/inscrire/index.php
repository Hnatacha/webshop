...
$inscriptions  = findByAllOrderBy('compte', $con, 'nom','prenom','email','mot_de_passe','datedeconfirmation','mobile','adresse');
    ...
    <div class="wrapper d-flex flex-column min-vh-100 bg-light">
      ...
      <div class="body flex-grow-1 px-3">
        <div class="d-flex flex-row-reverse">
          <a href="../../frontend/register.php" class="btn btn-primary" title="Ajouter un inscription">
          ...
          <table class="table table-striped">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><?=$inscription['datedeconfirmation']?></td>
                <td><?=$inscription['mobile']?></td>
                <td><?=$inscription['adresse']?></td>
                
                <td>
                  <span>
                  <a href="edit.php?id=<?=$inscription['id']?>" class="text-primary" title="Editer">
                        <i class="fa fa-edit" aria-hidden="true" data-id="<?=$inscription['id']?>" ></i>
                      </a>
                  <a href="#" class="text-danger ml-2" title="Supprimer" data-id="<?=$inscription['id']?>"  
                         data-nom="<?=$inscription['nom']?>" onclick="onSupprimer(event)">
                        <i class="fa fa-trash" aria-hidden="true" data-id="<?=$inscription['id']?>"  
                         data-nom="<?=$inscription['nom']?>" ></i>
                      </a>
                  </span></td>
              </tr>
              <?php
    ...
    <script>
      $('#')
       onSupprimer = (event) => {
             ...
             let nom = $(target).data("nom");
             
             let mBody = document.getElementById('supAccessMBody');
             mBody.innerHTML = '';
             let body = nom +'<br><br> Supprimer ?';
                 body += '<input type="text" hidden value="'+id+'" name="inscriptionID" id="inscriptionID">';
        ...
        request.done(function(response, textStatus, jqXHR) {
          ...
          if(response.success){
            ...
            toastr.success('inscription supprimé');
            setTimeout(() => {
                window.location.href = window.location.origin+'/webshop/admin/inscription/index.php';
              }, 1000);
