    ...
    <div class="wrapper d-flex flex-column min-vh-100 bg-light">
      ...
      <div class="body flex-grow-1 px-3">
        <div class="d-flex flex-row-reverse">
          <a href="edit.php" class="btn btn-primary" title="Ajouter un accessoire">
        ...
        <section class="section" id="products">
          <table class="table table-striped">
            ...
            <tbody>
              <?php
              foreach($accessoires as $k => $accessoire)
              {
              ?>
              <tr>
                <th scope="row"><?=$k?></th>
                ...
                <td>
                  <span>
                  <a href="edit.php?id=<?=$accessoire['id']?>" class="text-primary" title="Editer">
                        <i class="fa fa-edit" aria-hidden="true" data-id="<?=$accessoire['id']?>" ></i>
                      </a>
                  <a href="#" class="text-danger ml-2" title="Supprimer" data-id="<?=$accessoire['id']?>"  
                         data-nom="<?=$accessoire['nom']?>" onclick="onSupprimer(event)">
                        <i class="fa fa-trash" aria-hidden="true" data-id="<?=$accessoire['id']?>"  
                         data-nom="<?=$accessoire['nom']?>" ></i>
                      </a>
                  </span></td>
              </tr>
              <?php
      ...
      $('#')
       onSupprimer = (event) => {
             ...
             let body = nom +'<br><br> Supprimer ?';
                 body += '<input type="text" hidden value="'+id+'" name="accessoireID" id="accessoireID">';
        ...
        supprimer =  (event) => {
          let id = $('#accessoireID').val();
          var formData = new FormData();
          formData.append('_method', 'DELETE');
          formData.append('id', id);

          request = $.ajax({
          url: "accessoireController.php",
          type:  'POST',
          data: formData,
          cache: false,
          contentType: false,
        ...
        request.done(function(response, textStatus, jqXHR) {
          ...
          if(response.success){
            ...
            toastr.success('Accessoire supprimé');
            setTimeout(() => {
                window.location.href = window.location.origin+'/webshop/admin/accessoire/index.php';
              }, 1000);
